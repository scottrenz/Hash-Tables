class HashTable:
    ...
    def _entry(self, key):
        # get hash and index
        idx = hash(key) % self._size        # find entry by key
        p, q = self._buckets[idx], None
        while p and p.key != key:
            p, q = p.next, p        # index, entry, previous entry
        return idx, p, q
    def _ensure_capacity(self):
        ...
        if fill > self.ratio_expand:
            ...
            return        # reallocate buckets
        self._buckets = [None] * self._size        # store entries into new buckets
        p = self._list
        while p:
    ...
    def __setitem__(self, key, value):
        idx, p, _ = self._entry(key)        # set entry if key was found
        if p:
            p.value = value
        ...
        self._buckets[idx] = p        # store to list
        if self._list:
    ...
    def __delitem__(self, key):
        idx, p, q = self._entry(key)        # key not found
        if not p:
            return        # remove from bucket
        if q:
            q.next = p.next
        else:
            self._buckets[idx] = p.next        # remove from list
        if p.entry_next:
            p.entry_next.entry_prev = p.entry_prev
        if p.entry_prev:
            p.entry_prev.entry_next = p.entry_next
        else:
            self._list = p.entry_next        # shrink
        self._count -= 1
        self._ensure_capacity()
    def __iter__(self):
        p = self._list
...
table = HashTable()> # add random values
for _ in range(1000):
    key, value = np.random.randint(1000), np.random.rand()
    table[key] = value> len(table), table._size